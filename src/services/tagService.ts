...
export class TagService {
  ...
  async getAllTags(page: number = 1, pageSize: number = 20): Promise<{ tags: Tag[], total: number }> {
  ...
  // Create new tag
  async createTag(tagData: NewTag): Promise<Tag> {
  ...
  // Update tag
  async updateTag(id: string, updateData: Partial<NewTag>): Promise<Tag | null> {
  ...
  async searchTags(query: string, page: number = 1, pageSize: number = 20): Promise<{ tags: Tag[], total: number }> {
    const offset = (page - 1) * pageSize;
  ...
  async getTagUsageStats(id: string): Promise<{ gymCount: number, trainerCount: number } | null> {
  ...
  async getAllTagsWithStats(): Promise<(Tag & { gymCount: number, trainerCount: number })[]> {
    const tags = await db
      .select()
      .from(schema.tags)
      .orderBy(schema.tags.name_en);
    ...
    const tagsWithStats = await Promise.all(
      tags.map(async (tag) => {
        const stats = await this.getTagUsageStats(tag.id);
        return {
          ...tag,
